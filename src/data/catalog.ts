...
let apiCatalogPromise: Promise<Product[]> | undefined;
...
function filterProductList(products: Product[], { category = DEFAULT_CATEGORY, searchQ = '', minRating = 0, minPrice = 0, maxPrice = Infinity, sortBy = '' }: FilterOptions = {}): Product[] {
...
async function loadApiCatalog(): Promise<Product[]> {
...
export function filterCatalog(options: FilterOptions = {}): Product[] {
  return filterProductList(CATALOG_PRODUCTS, options);
}

export async function fetchCatalog(options: FilterOptions = {}): Promise<Product[]> {
  try {
...
export function getDisplayTotal({ total }: { total: number }): number {
  return total;
}

export function getProductById(id: number | string): Product {
...
export function searchCatalog(query: string, limit = 5): Product[] {
  const value = query.trim().toLowerCase();
  if (!value) return [];
  return CATALOG_PRODUCTS.filter((product) => productMatchesQuery(product, value)).slice(0, limit);
}
...
export async function fetchRelatedProducts(productId: number, limit = 6): Promise<Product[]> {
